            ...
            using (MySqlConnection connection = new MySqlConnection(Utilities.MySqlConnectionString))
            {
                try
                {
                    ...
                    for (int i = 0; i < dgvImport.Rows.Count; i++)
                    {
                        ...
                        _work_address = DataGridView_CellString(i, 19);
                        _date_hired = DataGridView_CellStringDate(i, 20);
                        _allocation = DataGridView_CellString(i, 21);

                        _verification_means = DataGridView_CellString(i, 22);
                        _verification_date = DataGridView_CellStringDate(i, 23);
                        _verification_status = DataGridView_CellString(i, 24);
                        _follow_up_date_1 = DataGridView_CellStringDate(i, 25);
                        ...
                        _application_status = DataGridView_CellString(i, 32);
                        _withdrawn_reason = DataGridView_CellString(i, 33);
                        _employment_status = DataGridView_CellString(i, 34);
                        _hired_date = DataGridView_CellStringDate(i, 35);
                        _submitted_documents_date = string.Empty;
                        _interview_date = string.Empty;
                        _not_hired_reason = DataGridView_CellString(i, 36);

                        _count = DataGridView_CellString(i, 37);
                        _no_of_graduates = DataGridView_CellString(i, 38);
                        _no_of_employed = DataGridView_CellString(i, 39);
                        _verification = DataGridView_CellString(i, 40);
                        _job_vacancies = DataGridView_CellString(i, 41);
